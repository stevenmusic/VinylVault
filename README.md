...
在搜尋框打幾個字（例如 `Ari`），就會即時列出符合的歌手；點一下，App 就自動從 [MusicBrainz](https://musicbrainz.org)（免費的公開音樂資料庫，不用申請金鑰）抓出他歷年來所有黑膠發行：發行年份、地區、廠牌、廠牌編號、規格，以及每一張的 **Side A / Side B 完整曲目**。抓進來之後再用 ❤ Want / ✓ Owned 標記自己的收藏狀態。
